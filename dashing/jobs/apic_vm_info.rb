...
SCHEDULER.every '20s' do
	vm_list = JSON.parse(`http_proxy="" proxy="" curl -s -X "GET" "http://10.52.208.38/app/api/rest?opName=userAPIGetTabularReport&opData=%7Bparam0:%221%22,param1:%22ACI%22,param2:%22VMS-T0%22%7D" -H "x-cloupia-request-key: D47D6DD47B99423D9E499848DDF6D0A9"`)
	...
	apic_points << { x: (counted_vm_count / 10), y: cpu_average }
	send_event('apic_convergence', points: apic_points)

	current_cost = (vm_cost_per_hour * counted_vm_count).round(2)
	send_event('apic_valuation', { current: current_cost, last: last_cost })
	send_event('apic_karma', { current: current_cost, last: last_cost })
